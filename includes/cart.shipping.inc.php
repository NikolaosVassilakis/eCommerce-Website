...
if(isset($_POST["save-shipping"])){
    ...
    if (emptyInputSAccount($sfirstname, $slastname, $saddress1, $scountry, $scity, $spostcode, $stelephone) !== false) {
        header("location: ../cart.php?error=emptyinputS");
        exit();
    }

    // SOLO LETRAS Y ESPACIO 
    if (invalidSDetails1($sfirstname) !== false) {
        header("location: ../cart.php?error=invalidsfirstname");
        exit();
    }
    if (invalidSDetails1($slastname) !== false) {
        header("location: ../cart.php?error=invalidslastname");
        exit();
    }
    if (invalidSDetails1($scountry) !== false) {
        header("location: ../cart.php?error=invalidscountry");
        exit();
    }
    if (invalidSDetails1($scity) !== false) {
        header("location: ../cart.php?error=invalidscity");
        exit();
    }
    if (invalidSDetails1($sstate) !== false) {
        header("location: ../cart.php?error=invalidsstate");
        exit();
    }

    // SOLO LETRAS NUMBEROS EPSACIO GUION HASH PUNTO Y COMA
    if (invalidSDetails2($saddress1) !== false) {
        header("location: ../cart.php?error=invalidsaddress1");
        exit();
    }
    if (invalidSDetails2($saddress2) !== false) {
        header("location: ../cart.php?error=invalidsaddress2");
        exit();
    }
    if (invalidSDetails2($scompany) !== false) {
        header("location: ../cart.php?error=invalidscompany");
        exit();
    }
    if (invalidSDetails2($sASU) !== false) {
        header("location: ../cart.php?error=invalidsasu");
        exit();
    }

    // SOLO NUMEROS LETRAS Y ESPACIO
    if (invalidSDetails3($spostcode) !== false) {
        header("location: ../cart.php?error=invalidspostcode");
        exit();
    }

    // SOLO NUMEROS LETRAS Y ESPACIO
    if (invalidSDetails4($stelephone) !== false) {
        header("location: ../cart.php?error=invalidstelephone");
        exit();
    }

    addShippingDetailsCart($conn, $sfirstname, $slastname, $scompany, $saddress1, $saddress2, $sASU, $scountry, $scity, $sstate, $spostcode, $stelephone, $id);


} else {
    header("Location: ../cart.php?error=noSPost");
    exit();
}
